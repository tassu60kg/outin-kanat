...
def update_reference(**data):
    sql = """UPDATE bib_references SET cite_key=:cite_key, type=:type, author=:author, title=:title, year=:year,
         publisher=:publisher, ISBN=:ISBN, journal=:journal, booktitle=:booktitle, volume=:volume, pages=:pages"""
    db.session.execute(text(sql), data)
    db.session.commit()